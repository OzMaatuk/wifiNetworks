...
#
# GITHUB: https://github.com/moha99sa/EvilAP_Defender
#
######################################################################
import MySQLdb
import time
...
def Conf_viewSSIDs():
    try:
	db = MySQLdb.connect(host='localhost', user=username, passwd=password, db='EvilAPDef')
        cursor = db.cursor()
...
def insert_ap(pkt):
	...
	for item in OUIs:
	    bssid = '"'+bssid+'"'
	    if not bssid.startswith('""'):
		    ssid = '"'+ssid+'"'
		    item = '"'+item+'"'
		    cmd = "insert into ssids_OUIs (mac, ssid, oui) values("+bssid+","+ssid+","+item+")"
		    #print cmd
		    cursor.execute(cmd)
		    db.commit()
